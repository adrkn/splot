...
var SPlot = /** @class */ (function () {
    ...
    SPlot.prototype.load = function () {
        ...
        for (var dx_1 = 0; dx_1 < this.area.count; dx_1++) {
            ...
            for (var dy_1 = 0; dy_1 < this.area.count; dy_1++) {
                groups[dx_1][dy_1] = [];
                this.indices[dx_1][dy_1] = [dx_1, dy_1];
                for (var i = 0; i < 4; i++) {
                    groups[dx_1][dy_1][i] = [];
                }
            }
        }
        utils_1.shuffleMatrix(this.indices);
        while (!isObjectEnds) {
    ...
    SPlot.prototype.render = function () {
        ...
        var ratioObjectGroup = (2 * this.camera.zoom * this.area.step) / (this.stats.minObjectSize + this.stats.maxObjectSize); // !!! max -> min
        var first = 0;
        var count = 0;
        //let zz = 0
        ...
        for (var i = 0; i < this.area.count; i++) {
            for (var j = 0; j < this.area.count; j++) {
                var _b = this.indices[i][j], dx = _b[0], dy = _b[1];
                if ((dx < this.area.dxVisibleFrom) || (dx > this.area.dxVisibleTo) || (dy < this.area.dyVisibleFrom) || (dy > this.area.dyVisibleTo))
                    continue;
