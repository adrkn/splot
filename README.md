...
Данные опции указываются в json-стилистике только при создании или первой настройке скаттерплота, т.к. контекст рендеринга WebGL создается только один раз. В качестве свойств экземпляра SPlot, они доступны только для чтения. По умолчанию опции WebGL настроены на максимальную производительность графики. Менять значения по умолчанию не рекомендуется и имеет смысл только в экспериментальных задачах.
...
- [ ] Более логичная файловая структура проекта.
